 ...
 *
 * POST /audit-entity
 * Body: { "entity_name": "...", "entity_type": "..." }
 * Dispatches audit-entity.yml GitHub Action to check codex card completeness.
 *
 * Required:
 *  - GH_TOKEN (secret)
 *  - GH_OWNER (var)
...
export default {
  async fetch(request, env) {
    ...
    // ── /audit-entity: dispatch audit-entity.yml via GitHub Actions ─
    if (isAuditEntity) {
      const entityName = body && body.entity_name ? String(body.entity_name).trim() : '';
      const entityType = body && body.entity_type ? String(body.entity_type).trim() : '';
      if (!entityName || !entityType) {
        return new Response(JSON.stringify({ error: 'Missing entity_name or entity_type' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      }
      ...
      if (!env.GH_TOKEN || !owner || !repo) {
        return new Response(JSON.stringify({
          ...
          missing: { GH_TOKEN: !env.GH_TOKEN, GH_OWNER: !owner, GH_REPO: !repo },
        }), {
          status: 500,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
        });
      ...
      const workflowFile = 'audit-entity.yml';
      const inputs = { entity_name: entityName, entity_type: entityType };
      ...
      if (!ghRes.ok) {
        ...
        let msg = '';
        try { const j = JSON.parse(raw); msg = j && j.message ? j.message : ''; } catch {}
